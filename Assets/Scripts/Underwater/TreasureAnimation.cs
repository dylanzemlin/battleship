...
public class TreasureAnimation : MonoBehaviour
{
    ...
    public void searchForTreasure(GameObject firingWeapon, Vector3 ammoTransform, Vector3 planetNormal, Vector3 planetCenter) {
        // if (Random.Range(0,50) == 1 && firingWeapon != null) {
        if (firingWeapon != null) {
            GameObject cannons = firingWeapon.transform.parent?.gameObject;
            if (cannons == null) {
                Debug.LogWarning("Could not access weapon's parent. firingWeapon.name: " + gameObject.name);
            } else {
                GameObject ship = cannons.transform.parent?.gameObject;
                    if (ship == null) {
                    Debug.LogWarning("Could not access weapon's parent's parent (AKA SHIP). Weapon's name: " + cannons.name);
                } else {
